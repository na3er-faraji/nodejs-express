const addProfile = ({ profileDB, profileEntity }) => {
    return async function postProfile(info) {
      ...
      const profileExists = await profileDB.searchProfile(result.email, result.nickname);
      ...
      const data = await profileDB.addProfile(result.name,result.email, result.nickname);
